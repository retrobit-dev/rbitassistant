...
4. Singkatan (`abbreviations`), dicocokkan per batas kata, **frasa terpanjang dulu** (`lampu senter` sebelum kata tunggal).
...
Belum ditangani (tercatat sebagai celah di golden set): `setengah`, termasuk idiom jam `setengah tujuh` = 6:30.
...
| `text` | teks *lazy*, opsional `max_words` | — | teks ternormalisasi |
...
|---|---|---|
| Salah eja / salah dengar | `buka watsap` | Tingkat 3: fuzzy resolver |
| Alias tidak persis | `telepon mama saya` | Tingkat 3 |
| Bahasa percakapan `-in` | `kecilin suaranya`, `matiin senternya` | Normalisasi (belum) atau tingkat 3 |
| Urutan kata bebas | `besok pagi jam lima bangunkan aku` | Tingkat 4: LLM |
| Judul tanpa kata kunci | `putar despacito` | Tingkat 4 |
| Batas kontak–pesan tanpa penanda | `kirim pesan ke ibu aku pulang telat` | Tingkat 4 |
| **Salah positif:** pertanyaan berbentuk perintah | `download lagu gratis dimana` → terbaca `search_play_store` | Tingkat 4 + konfirmasi |
...
Salah positif terakhir adalah yang paling berbahaya: salah positif mengeksekusi aksi yang tidak diminta, sedangkan celah biasa hanya berakhir di chatbot. Karena itu intent dengan akibat nyata (`call_contact`, `send_sms`, `send_whatsapp`) memakai `confirmation: always`.
